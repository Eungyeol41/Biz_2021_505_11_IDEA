...
<%--
form: form tag를 사용한 입력폼 디자인
security에서 form에 데이터를 입력하고 서버로 전송할 때는 반드시 POST 방식으로 전송해야 한다

GET 방식으로 데이터를 전송할 때는 인증(로그인 정보)와 관련된 어떤 기능을 수행할 수 없다
...
form으로 만든 입력변수 중에 _csrf.token 값에 서버로부터 전달받은 실제 token을 부착하는 코드를 만들어야 한다

이러한 절차가 순수 html로 만든 form에서는 상당히 번거로운 일이 된다

따라서 spring jsp 코드에서는 spring taglibs를 import하고 form:form 방식으로 HTML 코드를 작성한다

form:form 방식으로 HTML 코드를 작성하게 되면 자동으로 _csrf.token과 관련된 코드가 추가된다.
--%>
